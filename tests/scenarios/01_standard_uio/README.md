...
- **`addr[31:0]` (アドレスバス):** ソフトウェアがアクセスしようとしているレジスタの「番地」を示す32本の線です。C言語で `regs[4]` にアクセスすると、ここに `0x10` が流れてきます。
...
3. **アドレスオフセットの計算:** 
   DTS定義では `RST` はオフセット `0x10` ですが、C言語では 32ビット（4バイト）ポインタの配列として扱うため、インデックスは `0x10(16) / 4 = 4` になります。
