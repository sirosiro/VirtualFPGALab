...
graph TD
    ...
    subgraph "Emulation Layer (Shared Memory /tmp)"
        SHM["SHM File: vfpga_gpio_41200000"]
        subgraph "Register Map (DTS Defined)"
            DATA["0x00: DATA (CH1)"]
            TRI["0x04: TRI (CH1)"]
            DATA2["0x08: DATA2 (CH2)"]
            TRI2["0x0C: TRI2 (CH2)"]
        end
    end
...
1.  **UIO経由のレジスタアクセス**: `/dev/uio0` を通じて、物理アドレス `0x41200000` のレジスタをどのように操作するかを学びます。
